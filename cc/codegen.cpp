...
static bool check_clobber(cparse::Expression *expr) {
  if (dynamic_cast<cparse::ConstantExpression *>(expr) != nullptr) {
    return false;
  } else if (dynamic_cast<cparse::IdentifierExpression *>(expr) != nullptr) {
    return false;
  }
  return true;
}
...
void CodeGen::visit(cparse::BinaryExpression &bin_expr, bool rhs) {
  ...
  visit(*bin_expr.left);
  bool clobbers = check_clobber(bin_expr.right.get());
  if (clobbers) {
    out << "\tpush hl\n";
  }
